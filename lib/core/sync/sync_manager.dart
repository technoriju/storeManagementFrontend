...
enum SyncStatus { idle, syncing, error, offline, pending }

final syncManagerProvider =
    StateNotifierProvider<SyncManagerNotifier, SyncStatus>((ref) {
...
class SyncManagerNotifier extends StateNotifier<SyncStatus> {
  ...
  SyncManagerNotifier(this.db, this.apiClient, this.storage)
  ...
  Future<void> _pushChanges() async {
    ...
    for (final item in pendingItems) {
      try {
        await db
            .update(db.syncQueues)
            .replace(item.copyWith(status: 'SYNCING'));
  ...
  Future<void> _pullChanges() async {
    ...
    final response =
        await apiClient.dio.get('/api/sync/pull', queryParameters: {
  ...
  Future<void> _applyRemoteChange(Map<String, dynamic> change) async {
    ...
    if (entityType == 'PRODUCT') {
      if (operation == 'DELETE') {
        await (db.delete(db.products)..where((t) => t.id.equals(entityId)))
            .go();
      } else {
  ...
  void addToOutbox(String entityType, String entityId, String operation,
