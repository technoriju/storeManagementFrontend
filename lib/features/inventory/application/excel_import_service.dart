  ...
  Future<Map<String, dynamic>> parseExcelFile(String filePath) async {
    ...
    for (var table in excel.tables.keys) {
      ...
      for (int i = 1; i < rows.length; i++) {
        final row = rows[i];
        if (row.isEmpty || row.every((element) => element == null || element.value == null)) continue;
        ...
        // Validation logic
        List<String> errors = [];
        if (rowData['Product Code'] == null || rowData['Product Code'].toString().trim().isEmpty) {
          errors.add('Product Code is required');
        }
        if (rowData['Product Name'] == null || rowData['Product Name'].toString().trim().isEmpty) {
          errors.add('Product Name is required');
        }
        if (rowData['Unit'] == null || rowData['Unit'].toString().trim().isEmpty) {
