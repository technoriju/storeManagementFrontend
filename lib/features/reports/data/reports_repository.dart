...
class ReportsRepository {
  ...
  Future<List<Map<String, dynamic>>> getDailySales(DateTime startDate, DateTime endDate) async {
    ...
    final result = await db.customSelect(query, variables: [Variable.withString(start), Variable.withString(end)]).get();
    return result.map((e) => e.data).toList();
  }

  // Example: Top Products
  Future<List<Map<String, dynamic>>> getTopProducts(DateTime startDate, DateTime endDate) async {
    final start = startDate.toIso8601String();
    final end = endDate.toIso8601String();
